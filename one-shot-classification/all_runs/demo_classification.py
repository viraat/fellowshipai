...
def classification_run(folder,f_load,f_cost,ftype='cost'):
	...
	train_items = [f_load('./one-shot-classification/all_runs/'+f) for f in train_files]
	test_items  = [f_load('./one-shot-classification/all_runs/'+f) for f in test_files ]
...
if __name__ == "__main__":
	...
	for r in range(1,nrun+1):
		...
		if len(rs)==1:
			rs = '0' + rs
		perror[r-1] = classification_run('./one-shot-classification/all_runs/run'+rs, LoadImgAsPoints, ModHausdorffDistance, 'cost')
